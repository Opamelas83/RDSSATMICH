...
IntegratedLikelihoodCalculation1<-function (WD, DSSATD, CropName)
{

##Step 1. Get the information of integrated measurements from the processed evaluation file.
eval(parse(text=paste('Evaluation<-read.table("',DSSATD,'/EvaluateFrame_1.txt", header=TRUE,comment.char="")',sep = '')));
#print(Evaluation);
...
'), file ="',DSSATD,'/IntegratedLikelihoodTreatment_1_',i,'.txt", row.names = F, col.names = T, append = F)',sep="")));
