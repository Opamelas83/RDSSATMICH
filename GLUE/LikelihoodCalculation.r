...
LikelihoodCalculation<-function(WD, DSSATD, CropName, ParameterNames, RoundOfGLUE)
{
...
TreatmentNumber<-IntegratedLikelihoodCalculation1(WD, DSSATD, CropName);
} else
{
eval(parse(text=paste('source ("',WD,'/IntegratedLikelihoodCalculation2.r")',sep = '')));
TreatmentNumber<-IntegratedLikelihoodCalculation2(WD, DSSATD, CropName);
}
...
'<-read.table("',DSSATD,'/IntegratedLikelihoodTreatment_',RoundOfGLUE,'_',i,
...
if(RoundOfGLUE==1)
{
eval(parse(text=paste('RandomParameterSets<-read.table("',DSSATD,
...
eval(parse(text=paste('write.table(as.matrix(RandomParameterSets), file ="',DSSATD,
