...
OptimalParameterSet<-function (GLUEFlag, DSSATD, CropName, CultivarID, CultivarName, GenotypeFileName, TotalParameterNumber)
{
##1.Select the optimal parameter set.
if(GLUEFlag==1 | GLUEFlag==3)
{
eval(parse(text=paste('Optimal<-read.table("',DSSATD,
...
eval(parse(text=paste('GenotypeFilePath="',DSSATD,'/',GenotypeFileName,'.CUL"',sep = '')));
...
eval(parse(text=paste("NewGenotypeFilePath='",DSSATD,"/",CropName,"",CultivarName,".CUL'",sep = '')));
