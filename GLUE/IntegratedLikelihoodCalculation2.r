...
IntegratedLikelihoodCalculation2<-function (WD, DSSATD, CropName)
{

##Step 1. Get the information of integrated measurements from the processed evaluation file or File A.
eval(parse(text=paste('Evaluation<-read.table("',DSSATD,'/EvaluateFrame_2.txt", header=TRUE,comment.char="")',sep = '')));
#print(Evaluation);
...
eval(parse(text=paste('write.table(as.matrix(IntegratedLikelihoodMatrix), file ="',DSSATD,
'/IntegratedLikelihoodMatrix_Frame_2.txt", row.names = F, col.names = F, append = F)',sep = '')));

eval(parse(text=paste('IntegratedLikelihoodMatrixTable<-read.table("',DSSATD,
...
'), file ="',DSSATD,'/IntegratedLikelihoodTreatment_2_',i,'.txt", row.names = F, col.names = T, append = F)',sep="")));
