...
OUTweath_UYAB1901 <- OUT2 %>% slice(1:406) %>% 
                 mutate(TMAX = TMXD, TMIN = TMND, RAIN = PRED, 
                        PAR = PARD, WIND = WDSD) %>%
             dplyr::select(DAS, TMIN, TMAX, RAIN, SRAD, PAR, WIND)
...
HUMID2 <- HUMIDEC %>% mutate(TMAX = V4, TMIN = V3, RH2M = V6, DAS = row_number()) %>% 
                       dplyr::select(DAS,TMAX, TMIN, RH2M) 
...
HUMID2 <- HUMIDEC %>% mutate(TMAX = V4, TMIN = V3, RH2M = V6, DAS = row_number()) %>% 
                       dplyr::select(DAS,TMAX, TMIN, RH2M)  
OUTweath_UYUM1801 <- left_join(OUTweath_UYUM1801, HUMID2, by = c("TMIN", "TMAX"))
...
#Check estimation by env
```{r}
GLUEVAL2_envir <- read_csv("data/GLUEHWAM_VALana.csv")
GLUEVAL2_envir <- GLUEVAL2_envir %>% dplyr::select(source, Clone, HWAMS, HWAMM, HWAMMS, RAIN, RH2M, WIND, SRAD, TMIN, TMAX, PAR, year) %>% mutate(Environment = source)
#each environment for all clones
RES_envir <- GLUEVAL2_envir %>% dplyr::group_by(Environment) %>%
  dplyr::summarise(Obs = round(mean(HWAMM, na.rm = TRUE) / 1000, 1),
                   Sim = round(mean(HWAMS, na.rm = TRUE) / 1000, 1),
                   r = round(cor(HWAMS, HWAMM, use = "complete.obs"), 2),
                   rmse = round(rmse(HWAMS, HWAMM) / 1000, 1),
                   nrmse = round((nrmse(HWAMS, HWAMM) / 1000)*100),
                   d = round(d(HWAMS, HWAMM, na.rm = TRUE), 2),
                   `Total Clones` = length(Clone))
write.csv(RES_envir, "Result/GLUEHWAMenv.csv", quote = F, row.names = F)
#write.csv(RES_envir, "Result/GLUEHWAMenv_ObsSim.csv", quote = F, row.names = F)
hist(RES_envir$nrmse, breaks = c(0,10,20,40,60), freq = TRUE)
```

#Check estimation by Genotype
```{r}
GLUEVAL2_envir <- read_csv("data/GLUEHWAM_VALana.csv")
GLUEVAL2_envir <- GLUEVAL2_envir %>% dplyr::select(source, Clone, HWAMS, HWAMM, HWAMMS, RAIN, RH2M, WIND, SRAD, TMIN, TMAX, PAR, year) %>% mutate(Environment = source)
#each environment for all clones
RES_geno <- GLUEVAL2_envir %>% dplyr::group_by(Clone) %>%
  dplyr::summarise(r = round(cor(HWAMS, HWAMM, use = "complete.obs"), 2),
                   RMSE = round(rmse(HWAMS, HWAMM) / 1000, 1),
                   nRMSE = round((nrmse(HWAMS, HWAMM) / 1000)*100),
                   d = round(d(HWAMS, HWAMM, na.rm = TRUE), 2),
                   `Total ENV` = length(Environment))
write.csv(RES_geno, "Result/GLUEHWAMgenotype.csv", quote = F, row.names = F)
#jpeg(filename = "Result/Hist nrmse.jpeg")
ggplot(RES_geno, aes(x = nRMSE)) + 
  geom_histogram(aes(y = after_stat(count / sum(count))), binwidth = 10, boundary=0) + 
  ylab("Percentage") +
 # scale_y_continuous(breaks = seq(0,10,20,25))+
  scale_x_continuous(breaks = seq(0,25,10))+
stat_bin(
     binwidth = 10, boundary=0, geom = "text", color = "white",
    aes(y = after_stat(count / sum(count)), 
        label = scales::percent(after_stat(count / sum(count)))),
    position = position_stack(vjust = 0.5)
  ) +
  scale_y_continuous(labels = scales::percent) +
  ggtitle("Histogram of cultivar nRMSE") +
...
#PRvar$cos2           # represents the quality of representation for variables on the factor map. It’s calculated as the squared coordinates: var.cos2 = var.coord * var.coord. 
