...
Mydata file
```{r}
MyFILE <- SETT %>% select(accession_name,
                        rep_number,
                        plant_number,
...
MyFILEAdssat <- MyFILEA %>% select(accession_name, rep_number, plant_number, stem_nu, NODLT, Total_nod, LAIm, LAIX, "BR#S", "L#SX", "L#IR")
...
AnalMYFI2 <- AnalMYFI  %>% 
        ...
        dplyr::summarise_all(funs(mean))#(stem_nu_mean = mean(stem_nu, na.rm = T))
