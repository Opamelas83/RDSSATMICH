...
for(j in TRaits){
  for(i in TRials){
    ...
    Formula1 <- "Value ~ (1|repInTrial) + (1|blockInRep ) + (1|germplasmName)" 
...
```{r}
library(reactable)
reactable::reactable(results, groupBy = "Trial",
                     columns = list(VarG = colDef(format = colFormat(digits = 3, locales = "en-US")),
                                    VarE = colDef(format = colFormat(digits = 3, locales = "en-US")),
                                    H2 = colDef(format = colFormat(digits = 3, locales = "en-US")),
                                    Real = colDef(format = colFormat(digits = 3, locales = "en-US")),
                                    DvPrb = colDef(format = colFormat(digits = 3, locales = "en-US"))))
```
