...
## for R
REGR_R <- COMP1 %>% 
  select(Clone, R.Cor, GLUER.Cor) %>% 
           mutate(Clone = c(1:67),
                  BeforeGLUE = R.Cor,
                  AfterGLUE = GLUER.Cor) %>%
  select(Clone, beforeGLUE, afterGLUE)
...
## for R square 
COMP_Rsq <- COMP1 %>% select(R.sq) %>%
                     mutate("R²" = R.sq,
                            Statut = "BeforeGLUE")
COMP_RsqGLUE <- COMP1 %>% select(GLUER.Rsq) %>% 
                     mutate("R²" = GLUER.Rsq,
                            Statut = "afterGLUE") 
COMP_Rsq <- bind_rows(COMP_Rsq,COMP_RsqGLUE) %>% select(Statut, "R²")
...
## for rmse
COMP_RMSE <- COMP1 %>% select(R.rmse) %>%
                     mutate(RMSE = R.rmse,
                            Statut = "BeforeGLUE")
COMP_RMSEGLUE <- COMP1 %>% select(GLUER.rmse) %>% 
                     mutate(RMSE = GLUER.rmse,
                            Statut = "afterGLUE") 
COMP_RMSE <- bind_rows(COMP_RMSE,COMP_RMSEGLUE) %>% select(Statut, RMSE)
